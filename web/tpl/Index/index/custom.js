	jQuery(function($){	
		...
		if ($(window).width()<=640) {
			$("#categories").click(function(){
				$(".swipe-tips").fadeIn();
				setTimeout(function(){$(".swipe-tips").fadeOut();}, 1000);
			});
			$('.list').css({'left':0, 'transform':'translateX(-200px)'}).addClass('listAnimate').css('box-shadow','none').show();
			$("body").swipe( {
				threshold:200,
				allowPageScroll:"auto",
				swipeStatus:function(event, phase, direction, distance, duration,fingerCount) {
					console.log(distance);
					if (direction == 'right' && $(".list").hasClass('listAnimate')) {
						if (distance >= 100) {
							if (distance >= 150) {
								$('.list').show().removeClass('listAnimate').addClass('listShow').css("transform", "translateX(0px)");
								return;
							}
						}
					}
					if (direction == 'left') {
						if (distance >= 50) {
							$('.list').css("transform", "translateX(-200px)").addClass('listAnimate');
							return;
						}
					}
					if (phase == 'end' || phase == 'cancel') {
						if (!$('.list').hasClass('listShow')) {
